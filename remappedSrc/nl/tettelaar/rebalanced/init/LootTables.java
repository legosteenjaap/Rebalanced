...
public class LootTables {
	...
	public static void init() {
		Map<Identifier,ArrayList<LeafEntry>> lootPools = new HashMap<Identifier,ArrayList<LeafEntry>>();
		
		for (Pair<List<List<Identifier>>, List<Identifier>> book : RecipeAPI.getKnowledgeBooksLootTable()) {
			
			ArrayList<LeafEntry> itemEntries = new ArrayList<>();
			...
			for (List<Identifier> recipes : book.getLeft()) {
				...
				LootCondition.Builder lootCondition = AlternativeLootCondition.builder((EntityPropertiesLootCondition.builder(LootContext.EntityTarget.THIS, entityPredicate)), RandomChanceLootCondition.builder(0.2f));

				LeafEntry.Builder itemBuilder = ItemEntry.builder(Items.KNOWLEDGE_BOOK);
			...
			for (Identifier loottable : book.getRight()) {
				ArrayList<LeafEntry> itemEntry = lootPools.get(loottable);
				if (itemEntry == null) {
					lootPools.put(loottable, itemEntries);
				} else {
					List<LootPoolEntry> itemEntriesCopy = new ArrayList<>(itemEntries);
		...
		LootTableLoadingCallback.EVENT.register((((resourceManager, lootManager, id, supplier, setter) -> {
			if (lootPools.containsKey(id)) {
				...
				for (LootPoolEntry entry : lootPools.get(id)) {
